...
<template>
    <Teleport to="body">
        <Transition>
            <div ref="messageMenu" v-if="isShow" class="message-menu" :style="{ top: y + 'px', left: x + 'px' }">
                <n-menu dropdown-placement="bottom" :collapsed-width="18" :root-indent="0" :indent="0" :icon-size="16" mode="horizontal" :options="menuOption" accordion :on-update:value="onSelect"/>
            </div>
        </Transition>
    </Teleport>
</template>
